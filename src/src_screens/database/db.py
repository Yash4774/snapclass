...
def get_teacher_subjects(teacher_id):
    response = supabase.table('subjects').select("*,  attendance_logs(timestamp), subjects_students(count)").eq("teacher_id", teacher_id).execute()
    ...
    for sub in subjects:
        sub['total_students'] = sub.get('subjects_students', [{}])[0].get('count', 0)
...
def get_student_subjects(student_id):
    ...
    return [x['subjects'] for x in response.data]
